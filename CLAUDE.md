...
- `eval-run-3` — **n=3 repeats** (`EVAL_REPEATS`, mean±std + pairwise "stands out/inside noise"). Headline: **the version gaps were noise** — within-cell std 0.2–0.46 > every between-version delta (v1 4.62±0.33, v2 4.48±0.24, v3 4.60±0.26; all pairwise inside noise). What survives: v2 weakest in all 3 runs (consistent ordering), zh<en in nearly every cell. **Flipped DEFAULT_PROMPT_VERSION v2→v3** (tied with v1 on quality, better-maintained bilingual; not because it scored higher). See `notes/reports/eval-run-3.md`.
